...
def _world_axis_bounds_multi(objects, axis_idx):
    vals = []
    for obj in objects:
        for v in obj.data.vertices:
            vals.append((obj.matrix_world @ Vector(v.co))[axis_idx])
    if not vals:
        return 0.0, 1.0
...
def _build_plane_batch_multi(objects, axis_idx, cut_val):
    ...
    for obj in objects:
        mx = obj.matrix_world
        all_bbox.extend(mx @ Vector(c) for c in obj.bound_box)

    other = [a for a in range(3) if a != axis_idx]
    a, b = other
    pad = 0.5
    lo_a = min(c[a] for c in all_bbox) - pad
    hi_a = max(c[a] for c in all_bbox) + pad
    lo_b = min(c[b] for c in all_bbox) - pad
    hi_b = max(c[b] for c in all_bbox) + pad
...
class MESH_OT_bisect_interactive(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        self._objects = [o for o in context.selected_objects if o.type == 'MESH' and o.data.vertices]
        if not self._objects:
            self.report({'ERROR'}, "Select at least one mesh object with geometry")
            return {'CANCELLED'}

        self._axis_idx = 2  # Z

        amin, amax = _world_axis_bounds_multi(self._objects, self._axis_idx)
        self._axis_min = amin
        self._axis_max = amax
        ...
        # Precompute world-space triangle lists per object
        self._obj_tris = []  # list of (tri_centers, tri_verts) per object
        for obj in self._objects:
            ...
            for poly in obj.data.polygons:
                ...
                for i in range(1, len(face_verts) - 1):
                    tri_centers.append(center_world)
                    tri_verts.append((face_verts[0], face_verts[i], face_verts[i + 1]))
            self._obj_tris.append((tri_centers, tri_verts))
    ...
    def _rebuild_batches(self):
        ...
        self._batch_plane  = _build_plane_batch_multi(self._objects, axis_idx, cut_val)
    ...
    def _draw_hud(self, context):
        ...
        obj_str   = f"{obj_count} objects" if obj_count > 1 else self._objects[0].name
        line1 = f"Interactive Bisect  |  {obj_str}  |  Axis: {axis_name}  |  Cut: {self._cut_val:.4f} m  |  Fill cut face: {fill_str}"
    ...
    def _apply_bisect(self, context):
        ...
        for obj in self._objects:
            ...
            def _make_dup(obj=obj, original_cols=original_cols, original_world_mx=original_world_mx):
            ...
            def _cut_half(dup, plane_co_local, keep_above,
                          original_parent=original_parent, original_parent_mx=original_parent_mx):
                mesh = dup.data
                bm   = bmesh.new()
                bm.from_mesh(mesh)
            ...
            base_name = _strip_numeric_suffix(obj_name)
            name_a, name_b = _next_pair_names(base_name)
            dup_top.name = name_a
            if dup_top.data:
                dup_top.data.name = name_a
            dup_bottom.name = name_b
            if dup_bottom.data:
                dup_bottom.data.name = name_b

            results.append((name_a, name_b))

        axis_name = ('X', 'Y', 'Z')[axis_idx]
        if len(results) == 1:
            name_a, name_b = results[0]
            self.report({'INFO'}, f"Bisected along {axis_name} at {cut_val:.4f}m → {name_a}, {name_b}")
        else:
            self.report({'INFO'}, f"Bisected {len(results)} objects along {axis_name} at {cut_val:.4f}m")
        return {'FINISHED'}
